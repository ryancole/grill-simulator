...
// The lighter's flame: the little tongue of fire that stands at the nozzle while the
// primary action is held. A CPU particle system, unlike the lighter fluid it will one
// day light -- a flame is a handful of specks that rise and fade, with nothing to
// collide against and nothing for gameplay to query, so it needs neither PhysX nor the
// GPU (see Fluid, which needs both because its droplets splash and pool).
//
...
// at a walk trails behind the hand rather than dragging along rigidly with it.
//
...
public:
    ...
    // Emits this frame's particles from `origin`, the lighter's nozzle in world space.
    // Called every frame the flame is held on, and not at all otherwise: the emission
    // rate is banked across frames internally (see burn_carry_), so a flame looks the
    // same however long a frame ran.
    void Emit(DirectX::XMFLOAT3 origin, float dt);
