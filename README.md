...
Currently draws a triangle. It exists to prove the whole toolchain end to end:
the Agility SDK runtime loads, DXC compiles Shader Model 6.6, and the device
comes up with the debug layer attached.
...
shaders/        triangle.vs.cso, triangle.ps.cso    (Shader Model 6.6)
```
...
src/
  main.cpp                        window, message loop, file helpers
  renderer.cpp/.hpp               device, swapchain, PSO, the triangle
  agility_exports.cpp             D3D12SDKVersion / D3D12SDKPath
  dx_common.hpp                   ComPtr alias, ThrowIfFailed
shaders/
  triangle.hlsl                   VSMain + PSMain
```
